...
Control other devices  is the main function of the Programmable Logic Controller (PLC) in DENlab

Batteries and Back-up systems bring stability to an autonomous system

#### Week 8: Integration in the Electricity Network > Pros & Cons: Integration in the Electricity Network > Case: Centralized vs decentralized grids

There is already a lot of experience with traditional power systems, which means that most surely, this option will be cheaper than the implementation of microgrids. Furthermore, the technology required by microgrids is fairly new, which translates to more costs.

the implementation of microgrids with distributed generation requires a great number of forecast algorithms. In comparison, the control of a traditional power grid is simpler.

In this case, microgrids with smart grid concepts are more likely to have an efficient use of energy, since its implementation means that there is control over when some loads are used. This translates to better use of energy at all times.


The inclusion of intermittent renewable energy sources in microgrids adds an uncertainty factor to the supply of power. In the case of traditional centralized generation, there are several factors that ensure a more stable grid frequency, including the fact that the generation plants are controllable.



##### the conditions needed to ensure a stable electrical grid with distributed generation

 - Energy storage options 

 - Smart energy concepts such as demand side management 
